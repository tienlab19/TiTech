    ...
    @IBAction func onLoginBiometric(_ sender: UIButton) {
        ...
        if bioAuth.isTouchIdEnabledOnDevice() || bioAuth.isFaceIdEnabledOnDevice() {
            bioAuth.authenticateWithBiometrics {
                ...
                self.presenter.showAlert(error?.localizedDescription ?? "", actionTitle: "Đóng", action: nil)
            }
        } else {
            self.presenter.showAlert("Thiết bị không hỗ trợ hoặc bạn cần thiết lập sinh trắc học trước khi sử dụng tính năng này", actionTitle: "Đóng", action: nil)
        }
    }
}
